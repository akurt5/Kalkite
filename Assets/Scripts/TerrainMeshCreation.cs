...
public static class TerrainMeshCreation
{
    ...
    public static Mesh CreateQuad(Vector2 _Dimensions, Vector3 _Position, out Vector3[] _OutVertices, Vector3[] _InVertices)
    {
        Mesh mesh = new Mesh();
        ...
        for (int i = 0; i < 4; i++)
        {
            if (_InVertices[i] != new Vector3(-1, -1, -1))
            {
                if(_InVertices[i].x == -1)
                {
                    _InVertices[i].x = vertices[i].x;
                }
                if(_InVertices[i].y == -1)
                {
                    _InVertices[i].y = vertices[i].y;
                }
                if(_InVertices[i].z == -1)
                {
                    _InVertices[i].z = vertices[i].z;
                }
                
                vertices[i] = _InVertices[i];
            }
        }
